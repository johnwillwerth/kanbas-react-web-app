...
export default function CoursesNavigation() {
  ...
  return (
    <div id="wd-courses-navigation" className="wd list-group fs-5 rounded-0">
      {links.map((link) => (
        <Link
          key={link.path} to={link.path} className={`list-group-item border
            ${pathname.includes(link.label) ? "active text-black bg-white border-0" : "text-danger bg-white border-0"}`}>
          {link.label}
        </Link>
      ))}
    </div>
